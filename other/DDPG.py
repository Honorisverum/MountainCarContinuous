...
class Actor(object):
    def __init__(self, n_observation, n_action, name='actor_net'):
        ...
        with tf.variable_scope(self.name) as scope:
            ...
            train_op = tf.train.AdamOptimizer(self.learning_rate).apply_gradients(zip(var_grads,self.trainable_vars))
...
with tf.Session() as sess:
    ...
    while episode < max_episode:
        ...
        iteration += 1
        if done:
            print('Episode Reward: {:.5} , Episode: {:}'.format(episode_reward,episode_steps))
            obs = env.reset()
            episode += 1
            episode_reward = 0
            episode_steps = 0
